...
namespace blockoptimiser.DataAccessClasses
{
    public class OpexDataAccess : BaseDataAccess
    {
        ...
        public void Insert(Opex newOpex)
        {
            ...
            using (IDbConnection connection = getConnection())
            {
                String insertQuery = $"insert into Opex (ScenarioId, CostType, CostName, FilterType, FilterName, UnitType, UnitId, UnitName, IsUsed)" +
                    $" OUTPUT INSERTED.Id  " +
                    $" VALUES(@ScenarioId, @CostType, @CostName, @FilterType, @FilterName, @UnitType, @UnitId, @UnitName, @IsUsed)";
